...
procedure Mini_3D is
  ...
  begin
    ...
    GL.Enable (GL.Blend);
    GL.Set_Blend_Func (Src_Factor => GL.SRC_ALPHA, Dst_Factor => GL.ONE_MINUS_SRC_ALPHA);
  ...
  begin
    GLUT.Init;
    GLUT.Initialize_Display_Mode (GLUT.DOUBLE or GLUT.RGB or GLUT.DEPTH);
    main_size_x := 520;
    main_size_y := 420;
    GLUT.Initialize_Window_Size (Integer (main_size_x), Integer (main_size_y));
    GLUT.Initialize_Window_Position (120, 120);
    if GLUT.CreateWindow ("GLOBE_3D / Mini demo") = 0 then
      raise GLUT_Problem;
    end if;
