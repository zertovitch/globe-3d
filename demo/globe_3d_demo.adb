...
procedure GLOBE_3D_Demo is
  ...
  fairly_far_demo : constant := 10_000.0;

  package Stars is new GLOBE_3D.Stars_sky
    (num_stars => 5_000,
     far_side  => fairly_far_demo);
  ...
  procedure Set_Background_Color is
    ...
    use type GL.Clampf;
  begin
    GL.Clear_Color (fact * 0.2275, fact * 0.0745, fact * 0.4431, 0.0);  --  Dark violet
  ...
  begin
    GL.Viewport (0, 0, GL.Sizei (wp_width), GL.Sizei (wp_height));
    ...
    GL.Set_Shade_Model (GL.Smooth);  --  GL's default is SMOOTH, vs FLAT
    Set_Background_Color;
    GL.Clear_Accumulation_Buffer (0.0, 0.0, 0.0, 0.0);
  end Reset_for_3D;
  ...
  procedure Create_Objects (load : Boolean; doom3_custom : String) is
    ...
    procedure Load_Doom (name : String) is
      ...
      use G3D.Ident_Vectors, Ada.Strings, Ada.Strings.Fixed;
      cv : Cursor;
      id : G3D.Ident;
    begin
  ...
  is
    use G3D, G3D.Aux, G3DM, GL.Math, Ada.Strings.Unbounded;
    use type GL.Double;
    --
  ...
  begin
    ...
    alpha_correct := False;
    if gc (special_plus)  then var_alpha := var_alpha + seconds; alpha_correct := True; end if;
    if gc (special_minus) then var_alpha := var_alpha - seconds; alpha_correct := True; end if;
    if alpha_correct then
      if var_alpha < 0.0 then var_alpha := 0.0;
      elsif var_alpha > 1.0 then var_alpha := 1.0; end if;
      for f in bestiaire (beast_idx).face'Range loop
        bestiaire (beast_idx).face (f).alpha := var_alpha;
      end loop;
    end if;
  ...
  begin
    ...
    GLUT.Initialize_Window_Size (Integer (main_size_x), Integer (main_size_y));
    GLUT.Initialize_Window_Position (120, 120);
    if GLUT.CreateWindow
    ...
    GLUT.DisplayFunc (Main_Operations'Address);
    GLUT.IdleFunc    (Main_Operations'Address);
    GLUT.Devices.Initialize;

    if GLUT.CreateMenu (Menu'Address) = 0 then
      raise GLUT_Problem;
    end if;
    GLUT.AttachMenu (GLUT.MIDDLE_BUTTON);
    GLUT.AddMenuEntry (" * Full Screen", 1);
    GLUT.AddMenuEntry ("--> Exit (Esc)", 2);
  end Start_GLUTs;
  ...
  begin
    ...
    if foggy then
      ...
      GL.Fog (GL.FOG_END, 0.5 * fairly_far_demo);
    end if;
    Reset_for_3D (Integer (main_size_x), Integer (main_size_y));
    case smoothing is
      when hardware =>
  ...
  custom : Ada.Strings.Unbounded.Unbounded_String;

  procedure Get_Arguments is
    use Ada.Characters.Handling, Ada.Command_Line, Ada.Strings.Unbounded;
  begin
...
begin
  ...
  Create_Objects (switch (load), Ada.Strings.Unbounded.To_String (custom));
  if switch (dump) then
