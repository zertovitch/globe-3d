...
package body ogl.Textures is
   ...
   begin
      for Each in the_Points'range loop
         declare
            the_Vertex : ogl.geometry.Vertex renames the_Points (each);
         begin
   ...
   begin
      ...
      if to_Lower (Extension) = "bmp" then
         ogl.io.load (image_Filename,  ogl.io.BMP,  Integer (the_Texture.Name),  blending_hint => the_Texture.is_Transparent);

      elsif to_Lower (Extension) = "tga" then
         ogl.io.load (image_Filename,  ogl.io.TGA,  Integer (the_Texture.Name),  blending_hint => the_Texture.is_Transparent);
      else
   ...
   begin
      ...
      if unused_texture_List.Last > 0 then -- an existing unused texture has been found
       ...
       TexParameter ( TEXTURE_2D, TEXTURE_WRAP_T, CLAMP_TO_EDGE );
         ...
         ogl.Errors.log;  -- tbd: only for debug.
      end if;
