...
FOR FACE_SELECTOR USE (FRONT=>16#0404#,BACK=>16#0405#,FRONT_AND_BACK=>16#0408#);FOR FACE_SELECTOR'SIZE USE GL.ENUM'SIZE;SUBTYPE FACEENM IS FACE_SELECTOR;
PRAGMA OBSOLESCENT(FACEENM,"Prefer the new name, Face_Selector");PROCEDURE SET_CULL_FACE(MODE:FACE_SELECTOR);PROCEDURE CULLFACE(MODE:FACE_SELECTOR)RENAMES SET_CULL_FACE;
PRAGMA OBSOLESCENT(CULLFACE,"Prefer the new name, Set_Cull_Face");TYPE ORIENTATION IS(CLOCKWISE,COUNTER_CLOCKWISE);FOR ORIENTATION USE
(CLOCKWISE=>16#0900#,COUNTER_CLOCKWISE=>16#0901#);FOR ORIENTATION'SIZE USE GL.ENUM'SIZE;SUBTYPE ORIENTATIONENM IS ORIENTATION;PRAGMA OBSOLESCENT(ORIENTATIONENM,"Prefer the new name, Orientation");
CW:CONSTANT ORIENTATION:=CLOCKWISE;PRAGMA OBSOLESCENT(CW,"Prefer the new name, Clockwise");CCW:CONSTANT ORIENTATION:=COUNTER_CLOCKWISE;
PRAGMA OBSOLESCENT(CCW,"Prefer the new name, Counter_Clockwise");PROCEDURE SET_FRONT_FACE(MODE:ORIENTATION);PROCEDURE FRONTFACE(MODE:ORIENTATION)RENAMES SET_FRONT_FACE;
PRAGMA OBSOLESCENT(FRONTFACE,"Prefer the new name, Set_Front_Face");TYPE POLYGON_MODE_TYPE IS(POINT,LINE,FILL);FOR POLYGON_MODE_TYPE USE
(POINT=>16#1B00#,LINE=>16#1B01#,FILL=>16#1B02#);FOR POLYGON_MODE_TYPE'SIZE USE GL.ENUM'SIZE;SUBTYPE POLYGONMODEENM IS POLYGON_MODE_TYPE;
PRAGMA OBSOLESCENT(POLYGONMODEENM,"Prefer the new name, Polygon_Mode_Type");PROCEDURE SET_POLYGON_MODE(FACE:FACE_SELECTOR;MODE:POLYGON_MODE_TYPE);
PROCEDURE POLYGONMODE(FACE:FACE_SELECTOR;MODE:POLYGON_MODE_TYPE)RENAMES SET_POLYGON_MODE;PRAGMA OBSOLESCENT(POLYGONMODE,"Prefer the new name, Set_Polygon_Mode");
TYPE CLIPPLANEENM IS (CLIP_PLANE0,CLIP_PLANE1,CLIP_PLANE2,CLIP_PLANE3,CLIP_PLANE4,CLIP_PLANE5 );FOR CLIPPLANEENM USE (CLIP_PLANE0=>16#3000#,
CLIP_PLANE1=>16#3001#,CLIP_PLANE2=>16#3002#,CLIP_PLANE3=>16#3003#,CLIP_PLANE4=>16#3004#,CLIP_PLANE5=>16#3005# );FOR CLIPPLANEENM'SIZE USE GL.ENUM'SIZE;
PROCEDURE CLIPPLANE(PLANE:CLIPPLANEENM;EQUATION:GL.DOUBLEPTR);PROCEDURE GETCLIPPLANE(PLANE:CLIPPLANEENM;EQUATION:GL.DOUBLEPTR);TYPE DRAWBUFFERENM IS
(NONE,FRONT_LEFT,FRONT_RIGHT,BACK_LEFT,BACK_RIGHT,FRONT,BACK,LEFT,RIGHT,FRONT_AND_BACK,AUX0,AUX1,AUX2,AUX3 );FOR DRAWBUFFERENM USE (NONE=>16#0000#,
...
FRONT_AND_BACK=>16#0408#,AUX0=>16#0409#,AUX1=>16#040A#,AUX2=>16#040B#,AUX3=>16#040C# );FOR DRAWBUFFERENM'SIZE USE GL.ENUM'SIZE;PROCEDURE DRAWBUFFER(MODE:DRAWBUFFERENM);
...
PARAMS:FLOATPTR);PROCEDURE CLEARINDEX(C:GL.FLOAT);PROCEDURE CLEARCOLOR(RED_VALUE:GL.CLAMPF;GREEN_VALUE:GL.CLAMPF;BLUE_VALUE:GL.CLAMPF;
ALPHA_VALUE:GL.CLAMPF);PROCEDURE CLEAR(MASK:BITFIELD);PROCEDURE CLEARDEPTH(DEPTH_VALUE:GL.CLAMPD);PROCEDURE CLEARACCUM(RED_VALUE:GL.FLOAT;
GREEN_VALUE:GL.FLOAT;BLUE_VALUE:GL.FLOAT;ALPHA_VALUE:GL.FLOAT);PROCEDURE INDEXMASK(MASK:GL.UINT);PROCEDURE COLORMASK(RED_VALUE:GL_BOOLEAN;
GREEN_VALUE:GL_BOOLEAN;BLUE_VALUE:GL_BOOLEAN;ALPHA_VALUE:GL_BOOLEAN);PROCEDURE POINTSIZE(SIZE:GL.FLOAT);PROCEDURE LINEWIDTH(WIDTH:GL.FLOAT);
...
Y:GL.INT;WIDTH:GL.SIZEI;HEIGHT:GL.SIZEI);PROCEDURE PUSH_ATTRIB(MASK:BITFIELD);PROCEDURE PUSHATTRIB(MASK:BITFIELD)RENAMES PUSH_ATTRIB;PRAGMA OBSOLESCENT(PUSHATTRIB,"Prefer the new name, Push_Attrib");
PROCEDURE POP_ATTRIB;PROCEDURE POPATTRIB RENAMES POP_ATTRIB;PRAGMA OBSOLESCENT(POPATTRIB,"Prefer the new name, Pop_Attrib");PROCEDURE PUSHCLIENTATTRIB(MASK:BITFIELD);
...
PROCEDURE RESIZEBUFFERSMESA;PRIVATE FUNCTION GLGETSTRING(NAME:STRINGENM)RETURN UBYTEPTR;FUNCTION GETSTRING(NAME:STRINGENM)RETURN UBYTEPTR RENAMES GLGETSTRING;
...
PRAGMA IMPORT(STDCALL,CLEARINDEX,"glClearIndex");PRAGMA IMPORT(STDCALL,CLEARCOLOR,"glClearColor");PRAGMA IMPORT(STDCALL,GLCLEAR,"glClear");
...
PRAGMA IMPORT(STDCALL,CLEARACCUM,"glClearAccum");PRAGMA IMPORT(STDCALL,GLACCUM,"glAccum");PRAGMA IMPORT(STDCALL,SET_MATRIX_MODE,"glMatrixMode");
