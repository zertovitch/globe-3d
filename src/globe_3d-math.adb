...
package body GLOBE_3D.Math is
  ...
  function "*"(A : Matrix_33; x : Vector_3D) return Vector_3D is
    r : Real;
    Ax : Vector_3D;
    --  Caution: Matrix indices are 1-based, Vector indices are 0-based (GL)
  begin
    for i in 1 .. 3 loop
      r := 0.0;
      for j in 1 .. 3 loop
  ...
  is
     forward : constant Vector_3D :=
       Normalized ((center (0) - eye (0),  center (1) - eye (1),  center (2) - eye (2)));
  ...
  type Matrix_44_GL is array (0 .. 3, 0 .. 3) of aliased Real; -- for GL.MultMatrix
  pragma Convention (Fortran, Matrix_44_GL);  -- GL stores matrices columnwise

  M : Matrix_44_GL;
  --  M is a global variable for a clean 'Access and for setting once 4th dim
  pM : constant GL.doublePtr := M (0, 0)'Access;
...
begin
  --  Reset 4th dimension values in global matrix M.
  for i in 0 .. 2 loop
    M (i, 3) := 0.0;
    M (3, i) := 0.0;
  end loop;
