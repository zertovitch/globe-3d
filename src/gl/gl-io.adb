...
package body GL.IO is
        ...
        begin
          ...
          for i in buffer'Range loop
            buffer (i) :=
              Ubyte
                (SE_Buffer
                   (Stream_Element_Offset (i - buffer'First) + SE_Buffer'First));
          end loop;
        end;
      end if;
  ...
  procedure Get_Byte (b : in out Input_buffer; data_byte : out Ubyte) is
  begin
  ...
  is
    ...
    generic
      bit_depth : Positive;
    procedure GID_with_generic_bit_depth (image_descr : in out GID.Image_Descriptor);

    procedure GID_with_generic_bit_depth (image_descr : in out GID.Image_Descriptor) is
      ...
      --
      procedure Put_Pixel
        (red_value, green_value, blue_value : Primary_Color_Range;
         alpha_value                        : Primary_Color_Range)
      is
      pragma Warnings (off, alpha_value);  --  alpha is just ignored
      begin
        case bit_depth is  --  This test happens actually at compile time :-)
          when 32 =>
            the_Image.data (idx .. idx + 3) := (red_value, green_value, blue_value, alpha_value);
            idx := idx + 4;  -- Index on next pixel on the right, for next time.
          when 24 =>
            the_Image.data (idx .. idx + 2) := (red_value, green_value, blue_value);
            idx := idx + 3;  -- Index on next pixel on the right, for next time.
          when  8 =>
            the_Image.data (idx) := red_value;  --  = green = blue
    ...
    begin
      GID_Load_Image_Instanciated (image_descr, next_frame);
  ...
  procedure Screenshot (name : in String) is
    ...
    type loc_intPtr is new GL.intPointer;
    frame_viewport  : array (0 .. 3) of aliased GL.Int;
    function Cvt is new Ada.Unchecked_Conversion (System.Address, loc_intPtr);
    --  This method is functionally identical as GNAT's Unrestricted_Access
    --  but has no type safety (cf GNAT Docs)
    pragma No_Strict_Aliasing (loc_intPtr); -- recommended by GNAT 2005+
  begin
    --  Größe des Viewports abfragen --> Spätere Bildgrößenangaben
    GL.GetIntegerv (GL.VIEWPORT, GL.intPointer (Cvt (frame_viewport (0)'Address)));
    ...
    FileInfo.biWidth       := I32 (frame_viewport (2));
    FileInfo.biHeight      := I32 (frame_viewport (3));
    FileInfo.biPlanes      := 1;
    ...
    begin
      ...
      --
      Write_raw_BGR_frame (Stream (f), Integer (frame_viewport (2)), Integer (frame_viewport (3)));
      Close (f);
    exception
      when others =>
        Close (f);
  ...
  begin
    ...
    Write_Intel (bmp_size);                --  Suggested Buffer Size
    Write_Intel (U32'(0));                 --  Quality
    Write_Intel (U32'(0));                 --  Sample Size
    Write_Intel (U32'(0));
  ...
  begin
    ...
    rate := frame_rate;
    GL.GetIntegerv (GL.VIEWPORT, GL.intPointer (Cvt (frame_viewport (0)'Address)));
    width  := Positive (frame_viewport (2));
    height := Positive (frame_viewport (3));
